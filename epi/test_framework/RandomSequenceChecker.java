...
public final class RandomSequenceChecker {
    ...
    private static int computeDeviationMultiplier(double allowedFalseNegative, int numRvs) {
        ...
        final double[] ERROR_BOUNDS = {
                1 - 0.682689492137086, 1 - 0.954499736103642, 1 - 0.997300203936740, 1 - 0.999936657516334,
                1 - 0.999999426696856, 1 - 0.999999998026825, 1 - 0.999999999997440
        };
    ...
    private static boolean checkFrequencies(List<Integer> seq, int n,
        ...
        final Map<Integer, Integer> indivFreqs = new HashMap<>();
        for (Integer a : seq) {
            indivFreqs.put(a, indivFreqs.getOrDefault(a, 0) + 1);
        }

        // Check that there are roughly seq.size()/n occurrences of key.
        // By roughly we mean the difference is less than k_sigma.
        return indivFreqs
                .values()
                .stream()
                .allMatch(freq -> Math.abs(avg - freq) <= kSigmaIndiv);
    }

    private static boolean checkPairsFrequencies(List<Integer> seq, int n, double falseNegativeTolerance) {
        final List<Integer> seqPairs = new ArrayList<>();
    ...
    private static boolean checkTriplesFrequencies(List<Integer> seq, int n, double falseNegativeTolerance) {
        final List<Integer> seqTriples = new ArrayList<>();
    ...
    // seq is a sequence of integers, which should be in the range [0,n-1]. We assume n << seq.size().
    public static boolean checkSequenceIsUniformlyRandom(List<Integer> seq, int n, double falseNegativeTolerance) {
    ...
    public static List<Integer> computeCombinationIdx(List<Integer> list, int n, int k, int m) {
